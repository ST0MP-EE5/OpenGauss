...
Usage:
    ...
    gauss doctor               # Check configuration and dependencies
...
load_gauss_dotenv(project_env=PROJECT_ROOT / '.env')

# Point mini-swe-agent at ~/.gauss/ so it shares our config
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(get_gauss_home()))
...
from gauss_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from gauss_cli.config import get_env_path, get_gauss_home
    from gauss_cli.auth import get_auth_status
    ...
    auth_file = get_gauss_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def cmd_model(args):
    ...
    from gauss_cli.config import load_config, save_config, get_env_value, save_env_value
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from gauss_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    ...
    from gauss_cli.models import model_ids
...
def _model_flow_custom(config):
    ...
    """
    from gauss_cli.auth import _save_model_choice, deactivate_provider
    from gauss_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from gauss_cli.models import probe_api_models
    ...
    if model_name:
        ...
        if base_url or api_key:
            deactivate_provider()
        print(f"Endpoint saved. Use `/model` in chat or `{get_cli_command_name()} model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from gauss_cli.auth import _save_model_choice, deactivate_provider
    from gauss_cli.config import save_env_value, load_config, save_config
    from gauss_cli.models import fetch_api_models
...
def cmd_logout(args):
    ...
    from gauss_cli.auth import logout_command
    logout_command(args)


def cmd_status(args):
    """Show status of all components."""
    from gauss_cli.status import show_status
    show_status(args)
...
def cmd_version(args):
    """Show version."""
    print(f"Gauss v{__version__} ({__release_date__})")
    ...
    try:
        ...
        if behind and behind > 0:
            ...
            print(f"Update available: {behind} {commits_word} behind — run 'gauss update'")
        elif behind == 0:
            print("Up to date")
    except Exception:
        pass
...
def cmd_uninstall(args):
    """Uninstall Gauss."""
    from gauss_cli.uninstall import run_uninstall
    run_uninstall(args)


def _update_via_zip(args):
    """Update Gauss by downloading a ZIP archive.
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("gauss-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Gauss behaves unexpectedly.")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Gauss couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Gauss couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Gauss behaves unexpectedly.")
    return True
    ...
    try:
        ...
        if needs_migration:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print(f"Skipped. Run '{get_cli_command_name()} config migrate' later to configure.")
        else:
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``gauss -c Pokemon Agent Dev`` without quoting the
...
def main():
    """Main entry point for gauss CLI."""
    parser = argparse.ArgumentParser(
        prog="gauss",
        description="Gauss - focused Lean autoformalization workspace",
...
Examples:
    ...
    gauss status                    Show local status and auth state
    gauss doctor                    Diagnose local setup issues
    gauss update                    Update to the latest version
...
"""
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for the Gauss CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: gauss-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Gauss components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Gauss setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Gauss configuration"
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch gauss --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            gauss_bin = shutil.which("gauss")
            if gauss_bin:
                os.execvp(gauss_bin, ["gauss", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "gauss_cli.main", "--resume", selected_id],
                )
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Gauss to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Gauss",
        description="Remove Gauss from your system. Can keep configs/data for reinstall."
    )
    ...
    # e.g. ``gauss -c Pokemon Agent Dev`` → ``gauss -c 'Pokemon Agent Dev'``
