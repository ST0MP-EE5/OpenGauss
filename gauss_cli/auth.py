"""
Multi-provider authentication system for Gauss Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.gauss/auth.json with cross-process file locking.
...
from gauss_cli.branding import get_cli_command_name
from gauss_cli.config import get_gauss_home, get_config_path
...
DEFAULT_NOUS_CLIENT_ID = "gauss-cli"
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `{get_cli_command_name()} model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("GAUSS_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.gauss/auth.json
# =============================================================================
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `gauss logout`.
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.gauss/auth.json (not ~/.codex/)
#
# Gauss maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Gauss auth store (~/.gauss/auth.json).
    ...
    if not state:
        raise AuthError(
            f"No Codex credentials stored. Run `{get_cli_command_name()} model` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            f"Codex auth state is missing tokens. Run `{get_cli_command_name()} model` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            f"Codex auth is missing access_token. Run `{get_cli_command_name()} model` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            f"Codex auth is missing refresh_token. Run `{get_cli_command_name()} model` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Gauss auth store (~/.gauss/auth.json)."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Gauss auth store automatically.
    """
    refresh_token = tokens.get("refresh_token")
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Gauss's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to Gauss auth store")
            print("⚠️  Migrating Codex credentials to Gauss's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print(f"   Run `{get_cli_command_name()} model` to create a fully independent session.\n")
    ...
    refresh_timeout_seconds = float(os.getenv("GAUSS_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Gauss processes
    ...
    base_url = (
        os.getenv("GAUSS_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Gauss models — they're not reliable for agentic tool-calling
            if "gauss" in mid.lower():
                continue
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Gauss is not logged into Nous Portal.",
                            provider="nous", relogin_required=True)

        portal_base_url = (
            _optional_base_url(state.get("portal_base_url"))
            or os.getenv("GAUSS_PORTAL_BASE_URL")
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `gauss status` output."""
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    """Scan for credentials from other CLI tools that Gauss can reuse.

    Returns a list of dicts, each with:
      - provider: str   -- Gauss provider id (e.g. "openai-codex")
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `{get_cli_command_name()} model` to create a separate session",
        })
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from gauss_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'gauss model' or 'gauss setup' instead."""
    print(f"The '{get_cli_command_name()} login' command has been removed.")
    print(f"Use '{get_cli_command_name()} model' to select a provider and model,")
    print(f"or '{get_cli_command_name()} setup' for full interactive setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.gauss/auth.json."""

    # Check for existing Gauss-owned credentials
    try:
        existing = resolve_codex_runtime_credentials()
        print("Existing Codex credentials found in Gauss auth store.")
        try:
    ...
    if cli_tokens:
        ...
        print("Gauss will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            _save_codex_tokens(cli_tokens)
            base_url = os.getenv("GAUSS_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
            config_path = _update_config_for_provider("openai-codex", base_url)
            print()
            print("Credentials imported. Note: if Codex CLI refreshes its token,")
            print("Gauss will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — Gauss gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Gauss creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Gauss auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider("openai-codex", creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    print()
    print("Login successful!")
    print(f"  Auth state: ~/.gauss/auth.json")
    print(f"  Config updated: {config_path} (model.provider=openai-codex)")
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    print(f"Starting Gauss login via {pconfig.name}...")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("Gauss will use OpenRouter for inference.")
        else:
            print(f"Run `{get_cli_command_name()} model` or configure an API key to continue.")
    else:
