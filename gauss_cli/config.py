"""
Configuration management for Gauss.

Config files are stored in ``~/.gauss/`` by default:
- ``~/.gauss/config.yaml``  - All settings (model, toolsets, terminal, etc.)
- ``~/.gauss/.env``         - API keys and secrets

This module provides:
- gauss config          - Show current configuration
- gauss config edit     - Open config in editor
- gauss config set      - Set a specific value
- gauss config wizard   - Re-run setup wizard
"""
...
from gauss_cli.default_soul import DEFAULT_SOUL_MD
...
def get_legacy_gauss_home() -> Path:
    """Return the legacy Gauss home directory path."""
    return Path(os.getenv("GAUSS_HOME", Path.home() / ".gauss"))


def get_gauss_home() -> Path:
    """Return the active Gauss home directory."""
    explicit_gauss = os.getenv("GAUSS_HOME")
    if explicit_gauss:
        return Path(explicit_gauss).expanduser()
    return Path.home() / ".gauss"

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_gauss_home() / "config.yaml"
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into the active home if the user doesn't have one yet."""
...
def ensure_gauss_home():
    """Ensure the active Gauss home directory structure exists with secure permissions."""
    home = get_gauss_home()
...
OPTIONAL_ENV_VARS = {
    ...
    # ── Tool API keys ──
    "FIRECRAWL_API_KEY": {
        "description": "Firecrawl API key for web search",
    ...
    # GAUSS_TOOL_PROGRESS and GAUSS_TOOL_PROGRESS_MODE are deprecated —
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("GAUSS_TOOL_PROGRESS")
            old_mode = get_env_value("GAUSS_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from GAUSS_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from GAUSS_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("GAUSS_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from GAUSS_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: gauss config set <key> <value>")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.gauss/config.yaml."""
    import copy
    ensure_gauss_home()
...
#   openai-codex (OAuth — gauss login) — OpenAI Codex
#   nous         (OAuth — gauss login) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.gauss/config.yaml."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.gauss/.env."""
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.gauss/.env."""
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.gauss/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Gauss Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    handoff = config.get('cli', {}).get('handoff', {})
    if isinstance(handoff, dict):
        print(f"  Handoff:      {handoff.get('mode', 'auto')}")
    ...
    print(color("  gauss config edit     # Edit config file", Colors.DIM))
    print(color("  gauss config set <key> <value>", Colors.DIM))
    print(color("  gauss setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or not value:
            print("Usage: gauss config set <key> <value>")
            print()
            print("Examples:")
            print("  gauss config set model anthropic/claude-sonnet-4")
            print("  gauss config set terminal.backend docker")
            print("  gauss config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'gauss config migrate' to add them")
        ...
        print("  gauss config           Show current configuration")
        print("  gauss config edit      Open config in editor")
        print("  gauss config set <key> <value>   Set a config value")
        print("  gauss config check     Check for missing/outdated config")
        print("  gauss config migrate   Update config with new options")
        print("  gauss config path      Show config file path")
        print("  gauss config env-path  Show .env file path")
        sys.exit(1)
