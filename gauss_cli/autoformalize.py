...
def normalize_autoformalize_backend_name(value: str) -> str:
    ...
    if normalized in {"native", "direct", "opengauss", "opengauss-lean"}:
        normalized = DEFAULT_AUTOFORMALIZE_BACKEND
...
def _patch_lean4_cycle_tracker_compat(checkout_root: Path) -> None:
    tracker_path = checkout_root / "plugins" / LEAN4_CLAUDE_PLUGIN_NAME / "lib" / "scripts" / "cycle_tracker.sh"
    if not tracker_path.is_file():
        return
    try:
        original = tracker_path.read_text(encoding="utf-8")
    except OSError:
        return
    marker = "# OpenGauss compatibility: accept both --flag=value and --flag value forms."
    if marker in original:
        return
    legacy = """  for arg in \"$@\"; do
    case \"$arg\" in
...
"""
    modern = """  # OpenGauss compatibility: accept both --flag=value and --flag value forms.
  while [[ $# -gt 0 ]]; do
    case \"$1\" in
      --max-cycles=*) max_cycles=\"${1#*=}\" ;;
      --max-cycles) shift; max_cycles=\"${1:-}\" ;;
      --max-stuck=*) max_stuck=\"${1#*=}\" ;;
      --max-stuck) shift; max_stuck=\"${1:-}\" ;;
      --max-stuck-cycles=*) max_stuck=\"${1#*=}\"; name_stuck=\"--max-stuck-cycles\" ;;
      --max-stuck-cycles) shift; max_stuck=\"${1:-}\"; name_stuck=\"--max-stuck-cycles\" ;;
      --max-runtime=*) max_runtime=\"${1#*=}\" ;;
      --max-runtime) shift; max_runtime=\"${1:-}\" ;;
      --max-total-runtime=*) max_runtime=\"${1#*=}\"; name_runtime=\"--max-total-runtime\" ;;
      --max-total-runtime) shift; max_runtime=\"${1:-}\"; name_runtime=\"--max-total-runtime\" ;;
      --max-deep-per-cycle=*) max_deep_per_cycle=\"${1#*=}\" ;;
      --max-deep-per-cycle) shift; max_deep_per_cycle=\"${1:-}\" ;;
      --max-consecutive-deep=*) max_consecutive_deep=\"${1#*=}\" ;;
      --max-consecutive-deep) shift; max_consecutive_deep=\"${1:-}\" ;;
      --max-consecutive-deep-cycles=*) max_consecutive_deep=\"${1#*=}\"; name_consec_deep=\"--max-consecutive-deep-cycles\" ;;
      --max-consecutive-deep-cycles) shift; max_consecutive_deep=\"${1:-}\"; name_consec_deep=\"--max-consecutive-deep-cycles\" ;;
...
"""
    if legacy not in original:
        return
    tracker_path.write_text(original.replace(legacy, modern), encoding="utf-8")
...
def _ensure_lean4_checkout_assets(
    *,
    ...
    _patch_lean4_cycle_tracker_compat(checkout_root)
...
def _write_codex_instructions(
    *,
    ...
    template_text = _resolve_benchmark_template_text(
        base_environment,
        generic_env=AUTOFORMALIZE_INSTRUCTIONS_TEMPLATE_ENV,
    )
    if template_text:
        lines.extend(
            [
...
def _resolve_benchmark_template_text(
    base_environment: Mapping[str, str] | None,
    *,
    generic_env: str,
    legacy_env: str | None = None,
) -> str:
    template_text = _optional_text_file((base_environment or {}).get(generic_env))
    if template_text:
        return template_text
    if legacy_env is not None:
        return _optional_text_file((base_environment or {}).get(legacy_env))
    return ""
...
def _write_startup_context(
    *,
    ...
    template_text = _resolve_benchmark_template_text(
        base_environment,
        generic_env=AUTOFORMALIZE_STARTUP_TEMPLATE_ENV,
    )
    if template_text:
        lines.extend(
            [
