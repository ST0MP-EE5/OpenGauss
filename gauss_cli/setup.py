"""
Interactive setup wizard for Gauss Agent.
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Gauss Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Gauss using environment variables or config commands:")
    print_info("  gauss config set model.provider custom")
    print_info("  gauss config set model.base_url http://localhost:8080/v1")
    print_info("  gauss config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info(f"Run '{get_cli_command_name()} setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from gauss_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning(f"  Skipped (configure later with 'gauss setup')")


def _print_setup_summary(config: dict, gauss_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'gauss setup' to configure"))
    ...
    if disabled_tools:
        print_warning(
            f"Some tools are disabled. Run '{get_cli_command_name()} setup tools' to configure them,"
        )
        print_warning("or edit ~/.gauss/.env directly to add the missing API keys.")
        print()
    ...
    print(color("📁 All your files are in ~/.gauss/:", Colors.CYAN, Colors.BOLD))
    print()
    ...
    print(
        f"   {color('gauss config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('gauss config set <key> <value>', Colors.GREEN)}")
...
# between `gauss tools` and `gauss setup tools`).
...
def setup_model_provider(config: dict):
    ...
    if not has_any_provider:
        print_warning("An inference provider is required for Gauss to work.")
        print()
    ...
    if provider_idx == 0:  # Nous Portal (OAuth)
        ...
        try:
            from gauss_cli.auth import _login_nous, ProviderConfig
            import argparse
            ...
            print_info(f"You can try again later with: {get_cli_command_name()} model")
        ...
        config_path = (
            Path(os.environ.get("GAUSS_HOME", Path.home() / ".gauss")) / "config.yaml"
        )
        try:
            disk_cfg = {}
        ...
        if base_url:
            ...
            config_path = (
                Path(os.environ.get("GAUSS_HOME", Path.home() / ".gauss"))
                / "config.yaml"
            )
            try:
                disk_cfg = {}
        ...
        if api_key:
            ...
            from gauss_cli.auth import detect_zai_endpoint
    ...
    if selected_provider in _vision_backends:
        # If the user just selected a backend Gauss can already use for
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Gauss can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'gauss setup' or configure AUXILIARY_VISION_* settings")
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Gauss runs shell commands and code.")
...
def setup_agent_settings(config: dict):
    ...
    current_max = get_env_value("GAUSS_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("GAUSS_MAX_ITERATIONS", str(max_iter))
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `gauss setup tools` and `gauss tools` use the same flow:
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Gauss Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Gauss Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Gauss Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Gauss configured.")
        print()
        ...
        if choice == 0:
            ...
            func(config)
            save_config(config)
            _print_setup_summary(config, gauss_home)
            return
    else:
        # ── First-Time Setup ──
        print()
    ...
    print_info(f"Data folder:  {gauss_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'gauss config edit'")
...
def _run_quick_setup(config: dict, gauss_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info(f"Run '{get_cli_command_name()} setup' and choose 'Full Setup' to reconfigure,")
