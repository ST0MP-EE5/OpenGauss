"""
Unified tool configuration for Gauss Agent.

`gauss tools` and `gauss setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.gauss/config.yaml under
...
# They're still in _GAUSS_CORE_TOOLS (available at runtime if enabled),
...
TOOL_CATEGORIES = {
    "web": {
        ...
        "setup_title": "Configure Web Extraction",
        "setup_note": "Firecrawl enables direct web search and extraction. Browser tools stay available without it.",
        "icon": "🔍",
        "providers": [
            {
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning("    npm install failed - run manually: cd ~/.gauss/gauss-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in gauss-agent directory)")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `gauss tools` and `gauss setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Gauss Tool Configuration", Colors.CYAN, Colors.BOLD))
