...
from gauss_cli.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in gauss_cli/setup.py line 873.
    # A prior bug had 'get_codex_models' (wrong) instead of 'get_codex_model_ids'.
    from gauss_cli.codex_models import get_codex_model_ids as setup_import
...
def test_get_codex_model_ids_adds_forward_compat_models_from_templates(monkeypatch):
    monkeypatch.setattr(
        "gauss_cli.codex_models._fetch_models_from_api",
...
def test_model_command_uses_runtime_access_token_for_codex_list(monkeypatch):
    from gauss_cli.main import _model_flow_openai_codex

    captured = {}

    monkeypatch.setattr(
        "gauss_cli.auth.get_codex_auth_status",
    ...
    monkeypatch.setattr(
        "gauss_cli.auth._prompt_model_selection",
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a GaussCLI with minimal mocking."""
    ...
    with (
        ...
        cli = GaussCLI(model=model, **kwargs)
    return cli
...
class TestNormalizeModelForProvider:
    ...
    def test_default_model_replaced(self):
        ...
        with (
            patch("cli.get_tool_definitions", return_value=[]),
            patch.dict("os.environ", {"LLM_MODEL": "", "GAUSS_MAX_ITERATIONS": ""}, clear=False),
            patch.dict(_cli_mod.__dict__, {"CLI_CONFIG": _clean_config}),
        ):
            from cli import GaussCLI
            cli = GaussCLI()
