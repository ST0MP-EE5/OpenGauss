...
from gauss_cli.auth import _update_config_for_provider, get_active_provider
from gauss_cli.config import load_config, save_config
from gauss_cli.setup import setup_model_provider
...
def test_nous_oauth_setup_keeps_current_model_when_syncing_disk_provider(
    ...
    monkeypatch.setattr(
        "gauss_cli.setup.prompt_choice",
        lambda *args, **kwargs: next(prompt_choices),
    )
    monkeypatch.setattr("gauss_cli.setup.prompt", lambda *args, **kwargs: "")
    ...
    monkeypatch.setattr("gauss_cli.auth._login_nous", _fake_login_nous)
    monkeypatch.setattr(
        "gauss_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "gauss_cli.auth.fetch_nous_models",
...
def test_custom_setup_clears_active_oauth_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("gauss_cli.setup.prompt_choice", lambda *args, **kwargs: 3)
...
def test_codex_setup_uses_runtime_access_token_for_live_model_list(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("gauss_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("gauss_cli.auth.detect_external_credentials", lambda: [])
    monkeypatch.setattr("gauss_cli.auth._login_openai_codex", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "gauss_cli.auth.resolve_codex_runtime_credentials",
        lambda *args, **kwargs: {
