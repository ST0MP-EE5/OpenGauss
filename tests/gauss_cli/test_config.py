"""Tests for gauss_cli configuration management."""
...
class TestGetGaussHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("GAUSS_HOME", None)
            home = get_gauss_home()
            assert home == Path.home() / ".gauss"

    def test_env_override(self):
        with patch.dict(os.environ, {"GAUSS_HOME": "/custom/path"}):
            home = get_gauss_home()
            assert home == Path("/custom/path")


class TestEnsureGaussHome:
