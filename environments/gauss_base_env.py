"""
GaussAgentBaseEnv -- Abstract Base Environment for Gauss-Agent + Atropos

Provides the Atropos integration plumbing that all gauss-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via GaussAgentLoop
...
# Ensure the gauss-agent repo root is on sys.path so that imports like
...
# Load API keys from gauss-agent/.env so all environments can access them
...
class GaussAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for gauss-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of gauss toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: gauss, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
...
class GaussAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for gauss-agent Atropos integration.
    ...
    name: Optional[str] = "gauss-agent"
    env_config_cls = GaussAgentEnvConfig

    def __init__(
        self,
        config: GaussAgentEnvConfig,
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY gauss-agent tool (terminal, file, web,
