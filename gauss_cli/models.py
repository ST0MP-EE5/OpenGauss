...
Add, remove, or reorder entries here — both `gauss setup` and
`gauss` provider-selection will pick up the change automatically.
"""
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Gauss' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``gauss_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def provider_model_ids(provider: Optional[str]) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from gauss_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Gauss will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
