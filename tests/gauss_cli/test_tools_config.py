"""Tests for gauss_cli.tools_config platform tool persistence."""

from unittest.mock import patch

from gauss_cli.tools_config import (
    _get_platform_tools,
...
def test_toolset_has_keys_for_file_requires_no_configuration():
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/gauss-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
            "cli": ["web", "browser", "time", "github", "custom-mcp-server"]
        }
    }

    new_selection = {"web", "file"}

    with patch("gauss_cli.tools_config.save_config"):
    ...
    assert "file" in saved_toolsets
    assert "browser" not in saved_toolsets
...
def test_save_platform_tools_handles_empty_existing_config():
    ...
    with patch("gauss_cli.tools_config.save_config"):
        _save_platform_tools(config, "telegram", {"web", "file"})
